...
img = cv.imread("CRC-Prim-HE-07_APPLICATION.tif", -1)
show_img = img.copy()
# print type(img)

def split(patch, overlap):
    for i in range(0, img.shape[0], patch - overlap):
        for j in range(0, img.shape[1], patch - overlap):
            if (patch - overlap + i < img.shape[0] and patch - overlap + j < img.shape[1]):
                ...
                print 'test' + '_' + str(i) + '_' + str(j) + '_patch_' + str(patch) + '_overlap_'+str(overlap)+'.png'
                cv.imwrite('test' + '_' + str(i) + '_' + str(j) + '_patch_' + str(patch) + '_overlap_'+str(overlap)+'.png', img_array, [10, 10])
...
# cv.imwrite('test.jpg',img_array,[10,10])
# print img_array
# print img.shape
# print img.dtype
# print img.min()
